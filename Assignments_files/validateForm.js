...
function inputTextCheck()
{
    ...
    if ( element )
    {
      ...
      if (element.disabled)
      {
        ...
        if ( !val || !val.replace(/<p><\/p>/gi,'').trim() )
        {
            val='';
        }
...
nonceUtil.getNonceId = function(formId)
{
  ...
  if(formId && $s(formId) )
  {
    nonceId= $s(formId).elements['blackboard.platform.security.NonceUtil.nonce'];
  }
  if( !nonceId )
  {
   // take a cheap shot at retreiving the first nonceId on the page
   nonceId = document.getElementsByName( 'blackboard.platform.security.NonceUtil.nonce' )[0];
