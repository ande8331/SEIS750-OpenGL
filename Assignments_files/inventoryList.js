...
inventoryList.displayNotAllShownMessage = function(listId)
{
  ...
  new page.NestedInlineConfirmation("bad", msg, false, $s(listId), true, undefined, undefined, true);
};
    ...
    onSelectAllClick: function( event )
    {
      if ( this.optionsWhileSelectAll )
      {
        if(this.selectAllCheckbox.checked)
        {
          ...
          document.getElementById('selectCurrentItemNumber').innerHTML=checkboxes.length;
        }
        else
        {
          this.optionsWhileSelectAll.style.display="none";
    ...
    initialize: function( listContainer, listener )
    {
     ...
     // content/canvas item list
      ...
      genericList.initCheckboxesAndRadioButtons(this.containerId);
        ...
        // content List items
        if(this.selectAllCheckboxTop && this.selectAllCheckboxBot)
        {
        ...
        if(this.clearAllCheckboxTop && this.clearAllCheckboxBot)
        {
